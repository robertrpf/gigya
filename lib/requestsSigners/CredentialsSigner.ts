...
export class CredentialsSigner extends SimpleRequestSigner {
  ...
  public sign(request: GigyaRequest<SecretCredentials & SignedRequestParams>) {
    ...
    if (effectiveSecret && this._creds.secret) {
      requestParams.timestamp = Date.now();
      requestParams.nonce = this.createNonce();
      requestParams.sig = this.createRequestSignature(
        this._creds.secret,
  ...
  protected createRequestSignature(
    ...
    const queryString = Object.keys(requestParams)
      ...
      .join("&");
    const baseString = `${this._httpMethod.toUpperCase()}&${strictUriEncode(
      uri
    )}&${strictUriEncode(queryString)}`;
