...
import {
  ...
  PartnerSecret,
  PartnerSecretSigner,
} from "./requestsSigners/PartnerSecretSigner";
import {
  AnonymousRequestSigner,
  isAnonymous,
  NoCredentials,
} from "./requestsSigners/AnonymousRequestSigner";
import {
  AuthBearerSigner,
  isRSACreds,
  RSACredentials,
} from "./requestsSigners/AuthBearerSigner";
import {
  isSecretCredentials,
  SecretCredentials,
...
export type RequestParams = FormatJsonRequest &
...
export class Gigya {
  ...
  constructor(
    apiKey: string,
    dataCenter: DataCenter,
    credentials: RSACredentials
  );
  constructor(
    apiKeyOrProxy?: string | ProxyHttpRequest,
    protected _dataCenter: DataCenter = "us1",
    userKeyOrSecretOrCredentialsOrProxy?:
      | string
      | RSACredentials
      | ProxyHttpRequest,
    secret?: string
    ...
    if (typeof apiKeyOrProxy === "function") {
      ...
      switch (typeof userKeyOrSecretOrCredentialsOrProxy) {
        ...
        case "string":
          if (!secret) {
            creds = {
              secret: userKeyOrSecretOrCredentialsOrProxy as PartnerSecret,
            };
          } else {
            creds = {
   ...
   */
  protected async _request<R>(
    endpoint: string,
    userParams: BaseParams & { [key: string]: any },
    retries = 0
  ): Promise<GigyaResponse & R> {
    ...
    if (!request.skipSigning) {
      const signer =
        this.getSigner(userParams as CredentialsType) || this._signer;
      signer.sign(request);
    }
    ...
    try {
      ...
      if (
        e.errorCode === ErrorCode.GENERAL_SERVER_ERROR ||
  ...
  protected createRequestSignature(
    secret: string,
    uri: string,
    ...
    const queryString = Object.keys(requestParams)
      ...
      .map(
        (key) =>
          `${key}=${strictUriEncode((requestParams[key] || "").toString())}`
      )
      .join("&");
    const baseString = `${httpMethod}&${strictUriEncode(uri)}&${strictUriEncode(
      queryString
    )}`;
   ...
   */
  protected createErrorFromResponse(
